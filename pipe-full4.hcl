...
#First we add IADDQ as a valid instruction and then into the need_regids 
# array because the register is one of the operands, further into need_valC array because
# another operand is an immediate. Moreover, we employ the decoded register for rB in
# srcB, since the second operand is a register. Further, same register is employed as the
# final register of the ALU so we ensure d_dstE is D_rB for IADDQ. Next we pass the
#immediate val from execute stage to input stage A of ALU and valB to the second input to execute
#addition between the constant and the register. Next the same register stores the result.
#IADDQ is a mathematical instruction hence we set condition flags, thus we add 
